...
ProgramOptions::ProgramOptions() : desc_("Allowed options") {
    ...
    ("input,i", boost::program_options::value<std::string>(&inputFile_), "Input filepath")
    ("output,o", boost::program_options::value<std::string>(&outputFile_), "Output filepath")
    ("password,p", boost::program_options::value<std::string>(&password_), "Encrypt/decrypt password")
    ("command", boost::program_options::value<ProgramOptions::COMMAND_TYPE>(&command_), "Command: encrypt, decrypt, checksum")
    ;
}
...
bool ProgramOptions::Parse(int argc, char *argv[]) {
    try {
        ...
        else if(vm.count("input")&&vm.count("output")&&vm.count("password")&&vm.count("command")&&command_!=COMMAND_TYPE::CHECKSUM){
            return true;
        }
